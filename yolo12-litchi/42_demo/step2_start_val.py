...
model = YOLO(r"D:\JM\毕业设计\荔枝病虫检测系统\基于yolo12的农业害虫检测系统\yolo12-litchi\42_demo\runs\yolo12n_pretrained_6\train\weights\best.pt")

# 开始进行验证，验证的数据集为'A_my_data.yaml'，图像大小为640，批次大小为4，置信度分数为0.25，交并比的阈值为0.6，设备为0，关闭多线程（windows下使用多线程加载数据容易出现问题）
validation_results = model.val(data=r'D:\JM\毕业设计\荔枝病虫检测系统\基于yolo12的农业害虫检测系统\yolo12-litchi\ultralytics\cfg\datasets\A_DATA.yaml', imgsz=1080, batch=8, conf=0.1, iou=0.7, device="0", workers=0)
